...
function setupSocketHandlers(io) {
    ...
    gameNamespace.on('connection', (socket) => {
        ...
        // Track active challenges: Map<challengeId, challengeData>
        ...
        socket.on('challenge:send', (data) => {
            console.log('⚔️ Challenge:send received:', data);
            
            const { opponentId, opponentName, gameMode, timeControl, ranked } = data;
            const challengerId = socket.userId;
            const challengerName = socket.username;
            
            if (!challengerId || !opponentId) {
                socket.emit('challenge:error', { message: 'Invalid challenge data' });
                return;
            }
            ...
            console.log(`⚔️ Challenge created: ${challengeId}`);
            console.log(`   From: ${challengerName} (${challengerId})`);
            console.log(`   To: ${opponentName} (${opponentId})`);
            console.log(`   Settings: ${gameMode}, ${timeControl}, ranked=${ranked}`);
            
            // Find opponent's socket
            const opponent = onlinePlayers.get(opponentId.toString());
            
            if (opponent && opponent.socketId) {
                console.log(`📤 Sending challenge to opponent socket: ${opponent.socketId}`);
                
                // Send challenge to opponent
                gameNamespace.to(opponent.socketId).emit('challenge:received', challengeData);
            } else {
                console.log(`❌ Opponent ${opponentId} not found or offline`);
        ...
        socket.on('challenge:accept', (data) => {
            ...
            // Generate room for the game
            const roomCode = generateRoomCode();
            const gameId = `game-${Date.now()}`;
            
            console.log(`🎮 Creating game room: ${roomCode} (${gameId})`);
            
            // Parse time control
            let timePerPlayer = null;
            if (challenge.timeControl !== 'untimed') {
                const timeMatch = challenge.timeControl.match(/(\d+)min/);
                if (timeMatch) {
                    timePerPlayer = parseInt(timeMatch[1]) * 60; // Convert to seconds
                }
            }
            ...
            const accepter = onlinePlayers.get(challenge.opponentId);
            ...
            const responseData = {
                ...
                accepterName: challenge.opponentName
            };
            ...
            if (accepter && accepter.socketId) {
                gameNamespace.to(accepter.socketId).emit('challenge:accepted', {
                    ...responseData,
                    color: 'white'
                });
            }
            ...
            console.log(`✅ Challenge accepted and room created successfully`);
        });
